...
-- Enable Hammerspoon to run AppleScript for Hammerspoon Raycast extension functionality
...
-- -- Hotkey: Cmd + Alt + N = Open Clipboard Notion Link in New Tab
...
--   -- Validate that it is a Notion URL
--   if not url:match("notion%.so") then
--     hs.alert.show("Clipboard is not a Notion link")
--     return
--   end

--   -- Convert HTTPS link to Notion Deep Link
--   -- This replaces http:// or https:// with notion://
--   local deepLink = url:gsub("^https?://", "notion://")

--   local notion = hs.application.get("Notion")

--   -- Logic: If Notion is running and has a visible window, create a new tab first.
--   if notion and notion:isRunning() and notion:mainWindow() then
--     notion:activate()

--     -- Send Cmd + T to force a new tab in the current window
--     hs.eventtap.keyStroke({ "cmd" }, "t", 0)

--     -- Press Enter shortly after to confirm opening the empty tab
--     hs.timer.doAfter(0.1, function()
--       hs.eventtap.keyStroke({"cmd"}, "return", 0)
--       hs.eventtap.keyStroke({"cmd"}, "9", 0)

--       -- Wait for the tab to fully initialize before triggering the link
--       hs.timer.doAfter(0.3, function()
--         hs.urlevent.openURL(deepLink)
--       end)
--     end)
--   else
--     -- If Notion isn't open or has no windows, just opening the link will create the window
--     hs.urlevent.openURL(deepLink)
--   end
-- end)
